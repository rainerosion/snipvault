...
> 本文按用户能力描述当前工作树截至 2026-08-13 的实现，包括尚未发布的第一阶段效率层；v2.2.0 是已发布基线。异常行为和能力缺口统一链接到 [已知限制](known-limitations.md)，不在本文中包装为预期设计。
...
[App.tsx](../src/App.tsx) 使用 `React.lazy()` 延迟加载 `SnippetEditor`：只有选择片段或新建草稿才请求编辑器模块；无选择的浏览、搜索、同步和设置流程不会在空闲时预加载 CodeMirror。生产构建将 editor runtime、UI/services 和语言 parser family 分为有界独立 chunk，使首次进入编辑流程并行加载所需依赖，而不保留单个超大 editor chunk。
...
编辑器只安装一个 non-fallback syntax highlighter：项目共享的 GitHub 风格 `HighlightStyle`。UIW wrapper 使用 `theme="none"`，不会再附带另一套 GitHub syntax highlighter；GitHub 兼容的编辑器 chrome 由本地 `EditorView.theme()` 与语义 CSS token 提供。该 view theme 设置高度、字体、深浅 facet、滚动、光标、选区、换行和补全浮层；完整界面配色同时控制 editor surface、gutter、active gutter、光标、选区、匹配括号和 Canvas viewport。Canvas 背景从 minimap pane 的计算 token 读取，syntax token 颜色不随精选界面配色变化。

CodeMirror DOM 和 Canvas Codeglance 使用同一个共享 style，而且活动 Codeglance 直接消费当前 CodeMirror `EditorState` 的增量 `syntaxTree`，不启动另一轮一次性正文解析。因此刚输入、粘贴或受控替换的正文不会与旧 parser snapshot 配对；后台 parser 发布更完整的树时，即使用户没有再次按键，也会触发 Canvas 补绘。尚未出现在当前树中的区域、无 token 区域和 plaintext 都使用同一个编辑器默认前景色。
...
- 将当前 CodeMirror `EditorState` 的正文按行拆分，每行默认 4px 高。
- 直接复用该 live state 的增量语法树和唯一共享 `HighlightStyle` 取得 token 范围，并在共享 style module 已挂载后读取编辑器实际计算的颜色；不为活动 Codeglance 创建第二个 parser state，也不 force parse。
- 正文 identity 或 syntax-tree identity 改变时重绘：这覆盖刚输入/粘贴、受控正文或语言替换，以及 CodeMirror 后台 parser 的树发布；selection/focus-only 更新不重算。无 token、plaintext 或尚未被当前树覆盖的部分使用编辑器默认前景色。
- 使用行长度的 Q1/Q3/IQR 识别异常长行，避免单行把整个预览压缩得过窄。
- 空白字符仅保留水平几何位置，不绘制色条，避免缩略视图把缩进或分隔空格误读为深色代码。
- Canvas 宽度由用户拖动决定，高度至少覆盖内容。

Canvas 仍只压缩代码的几何形状，但语义 token 和颜色与当前 CodeMirror 编辑区共用同一个 live parser state 和同一条高亮管线：parser-backed 语言可覆盖嵌入式和跨行 token，StreamLanguage 则匹配编辑区自身产生的词法着色。独立 50 ms bounded parser 仅供没有 `EditorView` 的历史只读源码 renderer 使用。
