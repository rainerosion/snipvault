...
> 本文按用户能力描述 v2.1.2 截至 2026-08-02 已经实现的功能、交互状态和调用链。异常行为和能力缺口统一链接到 [已知限制](known-limitations.md)，不在本文中包装为预期设计。
...
- 将内容按行拆分，每行默认 4px 高。
- 用正则启发式分割注释、字符串、标识符、数字、标点和空白。
- 依据深浅主题选择颜色。
- 使用行长度的 Q1/Q3/IQR 识别异常长行，避免单行把整个预览压缩得过窄。
- Canvas 宽度由用户拖动决定，高度至少覆盖内容。

该 tokenizer 不复用 CodeMirror 的语言语法树，因此只提供近似颜色。
